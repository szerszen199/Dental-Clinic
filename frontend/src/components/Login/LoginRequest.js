...
export function makeLoginRequest(login, password, t) {
    axios.post(process.env.REACT_APP_BACKEND_URL + "auth/login", {
        ...
        if(response.status === 210){
            ...
            Cookies.set(process.env.REACT_APP_LANGUAGE_COOKIE, response.data.userInfoResponseDTO.language, {expires: jwtCookieExpirationTime, secure: true})
            Cookies.set(process.env.REACT_APP_DARK_MODE_COOKIE, response.data.userInfoResponseDTO.darkMode, {expires: jwtCookieExpirationTime, secure: true});
