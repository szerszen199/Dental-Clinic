...
(
    ...
    email                                     VARCHAR(100)       NOT NULL,  -- Adres email użytkownika, wykorzystywany do wysłania wiadomości z linkiem weryfikacyjnym
        CONSTRAINT acc_email_unique UNIQUE (email),
    ...
    pesel                                     CHAR(11),                     -- Numer pesel użytkownika
        CONSTRAINT acc_pesel_unique UNIQUE (pesel),
    ...
    last_successful_login                     TIMESTAMP,                 -- Pole reprezentujące datę ostatniego logowania użytkownika
    last_successful_login_ip                  VARCHAR(256),                -- Pole reprezentujące ip ostatniego logowania użytkownika
    last_unsuccessful_login                   TIMESTAMP,                 -- Pole reprezentujące datę ostatniego nieudanego logowania użytkownika
...
-- Sekwencja wykorzystywana przy tworzeniu pola klucza głównego tabeli account
...
(
    ...
    creation_date_time     TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,                    -- Data utworzenia tabeli
    created_by             BIGINT      NOT NULL,                                              -- ID konta które utworzyło tabelę
    created_by_ip          VARCHAR(256),
    modification_date_time TIMESTAMP,                                                       -- Data ostatniej modyfikacji tabeli
...
CREATE SEQUENCE access_levels_seq
    -- Sekwencja wykorzystywana przy tworzeniu pola klucza głównego tabeli access_level
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE CACHE 1;
...
(
    ...
    creation_date_time     TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP,-- Data utworzenia wizyty
    created_by             BIGINT             NOT NULL,                          -- Konto które stworzyło wizytę
    created_by_ip          VARCHAR(256),
    modification_date_time TIMESTAMP,                                          -- Data modyfikacji,
...
(
    ...
    creation_date_time     TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, -- Data stworzenia wiersza w tabeli
    created_by             BIGINT      NOT NULL,                           -- Id użytkownika
    created_by_ip          VARCHAR(256),
    modification_date_time TIMESTAMP,                                    -- Data ostatniej modyfikacji
...
(
    ...
    to_be_done             VARBINARY(80),                                          -- Tekst informujący co ma zostać wykonane na nastepnej wizycie
    version                BIGINT,                                          -- Wersja
        CONSTRAINT version_gr0 CHECK (version >= 0),
    creation_date_time     TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, -- Data utworzenia wiersza tabeli
    created_by             BIGINT      NOT NULL,                           -- Konto, które tworzyło wiersz tabeli
    created_by_ip          VARCHAR(256),
    modification_date_time TIMESTAMP,                                    -- Data ostatniej modyfikacji wiersza w tabelik
...
(
    ...
    medications            VARBINARY(80),                                          -- Tekst informujący o przepisanych lekarstwach
    version                BIGINT,                                          -- Wersja
        CONSTRAINT version_gr0 CHECK (version >= 0),
    creation_date_time     TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, -- Czas utworzenia wiersza w tabeli
    created_by             BIGINT      NOT NULL,                           -- Konto które utworzyło wiersz w tabeli
    created_by_ip          VARCHAR(256),
    modification_date_time TIMESTAMP,                                    -- Data ostatniej modyfikacji wiersza w tabeli
...
GRANT ALL PRIVILEGES ON accounts TO ssbd01admin;

GRANT ALL PRIVILEGES ON access_levels TO ssbd01admin;

GRANT ALL PRIVILEGES ON glassfish_auth_view TO ssbd01admin;
...
GRANT ALL PRIVILEGES ON appointments TO ssbd01admin;

GRANT ALL PRIVILEGES ON doctors_ratings TO ssbd01admin;
...
GRANT ALL PRIVILEGES ON medical_documentations TO ssbd01admin;

GRANT ALL PRIVILEGES ON documentation_entries TO ssbd01admin;

GRANT ALL PRIVILEGES ON prescriptions TO ssbd01admin;
